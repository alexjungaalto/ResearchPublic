#!/usr/bin/env python3
"""Effect of the GTV regularization parameter alpha (Fig. 1 and Table III of the paper).
...
alpha in {0,1,2,3,4} on the three 2-D synthetic datasets
(iso/aniso/var; n=10 nodes, m_i=200, ER p=0.7, K=3),
T=800 outer iterations, P=10 local passes; final xi_1 and
full xi_1 trajectories, mean over 10 seeds.
...
def exp_alpha(outdir, seeds, quick=False):
    """Table II + Fig. ablation row 1: effect of the regularization
    parameter alpha in {0,1,2,3,4} on the xi_1 trajectory and its
    final value after T=800 iterations (n=10, m_i=200, p=0.7, P=10).

    Expected per the paper: alpha=0 coincides with Local k-means (the
    GTV term vanishes); larger alpha drives xi_1 down.  Also counts
    safeguard triggers across all runs (spec E2)."""
    n_nodes, m_i, p, P = 10, 200, 0.7, 10
    T = 50 if quick else 800
    alphas = [0.0, 1.0, 2.0, 3.0, 4.0]
    rows, curves = [], []
    for ds in DATASETS:
        for seed in seeds:
            X, _ = gen_synthetic(ds, m_i * n_nodes, K=3, seed=seed)
            node_data = partition_uniform(X, n_nodes, seed)
            A = build_er_graph(n_nodes, p, seed)
            # W*: the centralized reference, computed once per
            # (dataset, seed) and shared by all (alpha, method) runs.
            mu_c = central_kmeans(node_data, 3, seed)
            for alpha in alphas:
                for name, ell in [("FedKM-NC", 1), ("FedKM-OT", 2)]:
                    rec = MetricRecorder(mu_c, A)
                    _, info = fed_kmeans(node_data, A, 3, ell, alpha=alpha,
                                         T=T, P=P, seed=seed, callback=rec)
                    rows.append(dict(dataset=ds, alpha=alpha, method=name,
                                     seed=seed, gcd=rec.gcd[-1],
                                     safeguards=info["safeguard_count"]))
                    curves.append(pd.DataFrame(dict(
                        dataset=ds, alpha=alpha, method=name, seed=seed,
    ...
    (pd.concat(curves)
       ...
       .reset_index()
       .to_csv(outdir / "effect_of_alpha_curves.csv", index=False))
    print("safeguard triggers total:", df["safeguards"].sum())
...
def main():
    ...
    exp_alpha(outdir, seeds, quick=args.quick)
