#!/usr/bin/env python3
"""Effect of local sample size and graph connectivity (Sec. V-C of the paper).
...
Final xi_1 vs m_i in {50,...,800} (T=300) and vs ER edge
probability p in {0,...,1} (T=200) for FedKM-NC, FedKM-OT,
Distributed k-means and Local k-means (alpha=eta=1, n=10).

Approximate full-settings runtime: 2-4 h single-threaded.
...
def _sweep(outdir, seeds, sweep_name, sweep_vals, fixed, quick=False):
    """Shared driver for the samplesize / connectivity sweeps
    (Fig. ablation rows 2-3): run all four methods at each sweep value
    and record the FINAL xi_1 (after T iterations)."""
    n_nodes, alpha, eta, K = 10, 1.0, 1.0, 3
    rows = []
    for ds in DATASETS:
        for seed in seeds:
            for val in sweep_vals:
                # The swept variable overrides its fixed default.
                m_i = val if sweep_name == "m_i" else fixed["m_i"]
                p = val if sweep_name == "p" else fixed["p"]
                T = 20 if quick else fixed["T"]
                X, _ = gen_synthetic(ds, m_i * n_nodes, K=K, seed=seed)
                node_data = partition_uniform(X, n_nodes, seed)
                A = build_er_graph(n_nodes, p, seed)
                mu_c = central_kmeans(node_data, K, seed)
                res = {}
                Ws, _ = fed_kmeans(node_data, A, K, 1, alpha, T=T, P=10,
                                   seed=seed)
                res["FedKM-NC"] = gcd(mu_c, Ws)
                Ws, _ = fed_kmeans(node_data, A, K, 2, alpha, T=T, P=10,
                                   seed=seed)
                res["FedKM-OT"] = gcd(mu_c, Ws)
                Ws = distributed_kmeans(node_data, A, K, eta=eta, T=T,
                                        seed=seed)
                res["Distributed"] = gcd(mu_c, Ws)
                res["Local"] = gcd(mu_c, local_kmeans(node_data, K, seed))
                for method, x1 in res.items():
                    rows.append(dict(dataset=ds, **{sweep_name: val},
                                     method=method, seed=seed, gcd=x1))
            print(f"[{sweep_name}] {ds} seed={seed} done", flush=True)
    df = pd.DataFrame(rows)
    agg = (df.groupby(["dataset", sweep_name, "method"])["gcd"]
             .apply(lambda v: pd.Series(mean_ci(v), index=["mean", "ci95"]))
             .unstack().reset_index())
    agg.to_csv(outdir / f"sweep_{sweep_name}.csv", index=False)


def exp_samplesize(outdir, seeds, quick=False):
    """Fig. ablation row 2: xi_1 after T=300 vs local sample size
    m_i in {50, 100, ..., 800} (alpha=eta=1, p=0.7, n=10).  The paper
    highlights the strong across-run dispersion of Distributed
    k-means here (label-mismatch conjecture)."""
    vals = [50, 200] if quick else list(range(50, 801, 50))
    _sweep(outdir, seeds, "m_i", vals, dict(p=0.7, T=300), quick)


def exp_connectivity(outdir, seeds, quick=False):
    """Fig. ablation row 3: xi_1 after T=200 vs ER edge probability
    p in {0, 0.1, ..., 1} (alpha=eta=1, m_i=200, n=10).  p=0 means no
    communication (should match Local), p=1 a complete graph."""
    vals = [0.2, 0.7] if quick else [round(x, 1) for x in np.arange(0, 1.01, 0.1)]
    _sweep(outdir, seeds, "p", vals, dict(m_i=200, T=200), quick)
...
def main():
    ...
    exp_samplesize(outdir, seeds, quick=args.quick)
    exp_connectivity(outdir, seeds, quick=args.quick)
