#!/usr/bin/env python3
"""Guarantee-covered update regimes of Proposition 1 (Table I of the paper).
...
Reruns the three synthetic datasets under the reported
configuration (gamma=1, P=10) and the two regimes covered
by Proposition 1 (gamma=1/P=1 and gamma=1/2/P=10), logging
final xi_1 and empirical descent violations of f_ell.

Approximate full-settings runtime: 30-60 min single-threaded.
...
CONFIGS = [
    ("default", 1.0, 10),   # the paper's configuration (not covered)
    ("regime-i", 1.0, 1),   # Prop. 1 regime (i)
    ("regime-ii", 0.5, 10), # Prop. 1 regime (ii)
]

REL_TOL = 1e-9  # relative tolerance for the monotone-descent check


def count_increases(obj):
    """Number of iterations where f_ell increased beyond tolerance."""
    diffs = np.diff(obj)
    scale = np.maximum(np.abs(obj[:-1]), 1.0)
    return int(np.sum(diffs > REL_TOL * scale))


def exp_regime(outdir, seeds, quick=False):
    T = 50 if quick else 200
    n, m, K, alpha, p = 10, 200, 3, 1.0, 0.7
    rows = []
    for ds in ["iso", "aniso", "var"]:
        for seed in seeds:
            seed = int(seed)
            X, _ = gen_synthetic(ds, n_samples=n * m, K=K, seed=seed)
            node_data = partition_uniform(X, n, seed=seed)
            A = build_er_graph(n, p, seed=seed)
            mu_c = central_kmeans(node_data, K, seed)
            for ell, meth in [(1, "FedKM-NC"), (2, "FedKM-OT")]:
                for cfg, gamma, P in CONFIGS:
                    Ws, info = fed_kmeans(node_data, A, K, ell,
                                          alpha=alpha, T=T, P=P,
                                          gamma=gamma, seed=seed,
                                          log_objective=True)
                    rows.append({
                        "dataset": ds, "method": meth, "config": cfg,
                        "gamma": gamma, "P": P, "seed": seed,
                        "gcd": gcd(mu_c, Ws),
                        "increases": count_increases(info["objective"]),
                    })
            print(f"  [regime] {ds} seed {seed} done")
    df = pd.DataFrame(rows)
    df.to_csv(outdir / "regime.csv", index=False)
    return df


def summarize_regime(df):
    print("\n== Experiment R: final GCD and descent violations ==")
    lines = []
    for (ds, meth, cfg), g in df.groupby(["dataset", "method", "config"],
                                         sort=False):
        mu, ci = mean_ci(g["gcd"].to_numpy())
        viol = int(g["increases"].sum())
        nruns = len(g)
        print(f"  {ds:6s} {meth:9s} {cfg:9s}  "
              f"GCD {mu:.3e} +/- {ci:.3e}   "
              f"descent violations {viol} (over {nruns} runs)")
        lines.append((ds, meth, cfg, mu, ci, viol))
    return lines


def write_regime_table(df, path):
    """LaTeX table body (single-column layout): rows = dataset x
    method, columns = the three configurations, entries = final log10
    GCD (mean +/- CI)."""

    def cell(ds, meth, cfg):
        g = df[(df.dataset == ds) & (df.method == meth) & (df.config == cfg)]
        logs = np.log10(np.maximum(g["gcd"].to_numpy(), 1e-300))
        mu, ci = mean_ci(logs)
        return f"${mu:.2f}\\pm{ci:.2f}$"

    ds_macro = {"iso": "$\\datasetiso$", "aniso": "$\\datasetaniso$",
                "var": "$\\datasetvar$"}
    with open(path, "w") as f:
        f.write("% Auto-generated by NumExpXu/regime_and_communities.py"
                " (exp_regime).\n")
        f.write("% Final log10 GCD (mean +/- 95% CI, 10 seeds, T=200).\n")
        f.write("\\begin{tabular}{ll ccc}\n\\toprule\n")
        f.write("& & \\makecell{$\\dampfac=1$\\\\$P=10$ (rep.)}"
                " & \\makecell{$\\dampfac=1$\\\\$P=1$ (i)}"
                " & \\makecell{$\\dampfac=\\tfrac{1}{2}$\\\\$P=10$ (ii)}"
                " \\\\\n\\midrule\n")
        for ds in ["iso", "aniso", "var"]:
            for k, meth in enumerate(["FedKM-NC", "FedKM-OT"]):
                lead = ds_macro[ds] if k == 0 else ""
                cells = [cell(ds, meth, cfg) for cfg, _, _ in CONFIGS]
                f.write(f"{lead} & {meth[-2:]} & " + " & ".join(cells)
                        + " \\\\\n")
        f.write("\\bottomrule\n\\end{tabular}\n")
    print(f"  wrote {path}")
...
def main():
    ...
    df = exp_regime(outdir, seeds, quick=args.quick)
    summarize_regime(df)
    write_regime_table(df, outdir / "regime_table.tex")
